...
const definition = JSON.parse(
  readFileSync(
    new URL("../../contracts/capability/v1/examples/read-definition.json", import.meta.url),
  ),
);
...
function request(overrides = {}) {
  return buildAuthorizationRequest({
    ...
    normalized_input: { include: ["health"] },
    risk: "low",
...
function evaluation(req, statements) {
  return {
    ...
    authorization_request_id: req.authorization_request_id,
    request_digest: req.request_digest,
    policy: structuredClone(req.policy),
    evaluator_ref: "impl_evaluator_reference001",
...
function statement(effect, reason_code, extra = {}) {
  return {
    statement_ref: `stmt_${effect}_${reason_code}`,
    effect,
    reason_code,
    constraints: [],
    obligations: [],
...
function decide(req, evaln, id = "decision_example001") {
  return combineAuthorization({
    request: req,
    evaluation: evaln,
    decision_id: id,
    issued_at: "2026-08-03T00:00:02Z",
...
test("an exact explicit allow authorizes", () => {
  ...
  assert.deepEqual(validateAuthorizationRecord("decision", decision), {
    valid: true,
    diagnostics: [],
  });
});
...
test("explicit deny overrides allow independent of order", () => {
  ...
  for (const statements of [
    ...
    assert.equal(decision.basis, "explicit");
...
test("multi-resource authorization is all-or-nothing", () => {
  const req = request({
    resource: { kind: "node", refs: ["node-a", "node-b"], attributes_digest: zero },
  });
  const evaln = evaluation(req, [
    statement("allow", "policy_allow_a", { resource_refs: ["node-a"] }),
  ]);
...
test("constraints intersect and obligations accumulate at strongest value", () => {
  const req = request();
  const evaln = evaluation(req, [
    statement("allow", "policy_allow_one", {
      constraints: [{ type: "output_fields", value: ["health", "uptime"] }],
      obligations: [{ type: "redaction_profile", value: "standard" }],
    }),
    statement("allow", "policy_allow_two", {
      constraints: [{ type: "output_fields", value: ["health"] }],
      obligations: [{ type: "redaction_profile", value: "strict" }],
    }),
  ]);
  const decision = decide(req, evaln);
...
test("empty constraint intersection is indeterminate deny", () => {
  ...
  const evaln = evaluation(req, [
    ...
    statement("allow", "policy_allow_two", {
      constraints: [{ type: "output_fields", value: ["uptime"] }],
    }),
  ]);
  const decision = decide(req, evaln);
...
test("one-shot enforcement rejects replay and every changed binding", () => {
  ...
  const variants = [
    { subject: { ...req.subject, ref: "subject_other001" } },
    {
      resource: {
        kind: "node",
        refs: ["node-other"],
        attributes_digest: req.resource.attributes_digest,
      },
    },
  ...
  for (const [index, change] of variants.entries()) {
    ...
    assert.equal(
      createDecisionEnforcer()({ decision, request: changed, now: "2026-08-03T00:00:03Z" }).allowed,
      false,
    );
  }
  ...
  assert.deepEqual(enforce({ decision, request: req, now: "2026-08-03T00:00:04Z" }), {
    allowed: false,
    code: "decision_already_consumed",
  });
});

test("approval obligations require an exact lifecycle-derived receipt", () => {
  const req = request();
  const evaln = evaluation(req, [
    statement("allow", "policy_allow_read", {
      obligations: [{ type: "approval_required", value: "standard" }],
    }),
  ]);
  const decision = decide(req, evaln);
  assert.deepEqual(
    createDecisionEnforcer()({ decision, request: req, now: "2026-08-03T00:00:03Z" }),
    { allowed: false, code: "obligation_pending" },
  );
  assert.deepEqual(
    createDecisionEnforcer()({
      decision,
      request: req,
      now: "2026-08-03T00:00:03Z",
      obligation_receipts: [
        { type: "approval_required", value: "standard", decision_id: "decision_other001" },
      ],
    }),
    { allowed: false, code: "obligation_pending" },
  );
  ...
  assert.deepEqual(
    createDecisionEnforcer()({
      ...
      now: "2026-08-03T00:00:03Z",
      obligation_receipts: [receipt],
      approval_receipt_verifier: (candidate) => candidate === receipt,
    }),
...
test("a constraint without its registered handler fails closed", () => {
  ...
  const evaln = evaluation(req, [
    statement("allow", "policy_allow_read", {
      constraints: [{ type: "max_output_bytes", value: 1024 }],
    }),
  ]);
  const decision = decide(req, evaln);
  assert.deepEqual(
    createDecisionEnforcer()({ decision, request: req, now: "2026-08-03T00:00:03Z" }),
    { allowed: false, code: "constraint_unenforceable" },
  );
  assert.equal(
    createDecisionEnforcer()({
      decision,
      request: req,
      now: "2026-08-03T00:00:03Z",
      constraint_handlers: { max_output_bytes: (value) => value === 1024 },
    }).allowed,
    true,
  );
  assert.deepEqual(
    createDecisionEnforcer()({
      ...
      constraint_handlers: {
        max_output_bytes: () => {
          throw new Error("untrusted constraint handler failure");
        },
      },
    }),
    { allowed: false, code: "constraint_unenforceable" },
...
test("invalid current time and fractional time windows fail closed", () => {
  const req = request();
  const evaln = evaluation(req, [
    statement("allow", "policy_allow_read", {
      constraints: [
        {
          type: "time_window",
          value: {
            not_before: "2026-08-03T00:00:02.1Z",
            not_after: "2026-08-03T00:00:10Z",
          },
        },
      ],
    }),
  ...
  const decision = decide(req, evaln);
  assert.deepEqual(createDecisionEnforcer()({ decision, request: req, now: "not-a-time" }), {
    allowed: false,
    code: "decision_stale",
  });
  assert.deepEqual(
    ...
    { allowed: false, code: "constraint_failed" },
  );
});
...
test("sanitized evidence contains digests, not policy or attribute values", () => {
  ...
  const evaln = evaluation(req, []);
  const decision = decide(req, evaln);
  const evidence = buildAuthorizationEvidence({
    evidence_id: "evidence_example001",
    request: req,
    decision,
    evaluation: evaln,
    enforcement: "denied",
  });
  assert.deepEqual(validateAuthorizationRecord("evidence", evidence), {
    valid: true,
    diagnostics: [],
  });

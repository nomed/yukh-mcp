...
test("non-loopback binding requires an explicit host allowlist", () => {
  assert.throws(() => loadRuntimeConfig({ YUKH_HOST: "0.0.0.0" }), {
    name: "TypeError",
    message: "YUKH_ALLOWED_HOSTS is required for a non-loopback bind",
  });
});

test("invalid configuration reports fields without their values", () => {
  assert.throws(
    () => loadRuntimeConfig({ YUKH_PORT: "secret-value", YUKH_ALLOWED_ORIGINS: "not a url" }),
    {
      name: "TypeError",
      message: "YUKH_PORT must be an unsigned base-10 integer",
    },
  );
  assert.throws(() => loadRuntimeConfig({ YUKH_ALLOWED_ORIGINS: "not a url" }), {
    name: "TypeError",
    message: "invalid runtime configuration: allowedOrigins.0",
  });
});
